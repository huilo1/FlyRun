...
export async function loadGraph(progress = () => {}, notice = () => {}) {
  ...
  let cache;
  try {
    cache = await globalThis.caches?.open('malecns-verified-data-v1');
  } catch {
  ...
  async function unpack(file, hash) {
    ...
    if (!bytes) {
      bytes = await requestBytes(url, { notice, fetcher: pacedFetch });
      if (hash && (await digest(bytes)) !== hash)
        throw Error('A downloaded data file failed its checksum. Please retry loading.');
    }
    let unpacked;
    try {
      ...
      throw Error('A data file could not be decompressed. Retry loading to download it again.');
    }
    try {
  ...
  notice('Загрузка списка нейронов…');
  const neurons = JSON.parse(new TextDecoder().decode(await unpack(manifest.metadata, manifest.metadataSha256)));
  progress(0.03);
  const graph = {
    n: manifest.neurons,
    neurons,
